...
int** fillArray(unsigned int numRows, unsigned int numCols)
{
	...
	My2DArray = new int*[numRows];//this puts an array of pointer to integers in My2DArray
	...
	for (int i = 0; i < numRows; i++) {
		...
		My2DArray[i] = new int[numCols];//this allocates an array of ints and assigns it to the current row of the existing array
...
void deleteArray(int** A, int size)
{
	...
	for (int i = 0;i < size;i++)//goes through the first array and dealocates each collumn
	{
		delete [] A[i];//deletes each column
	}
	delete[]A;//deletes the whole thing
}